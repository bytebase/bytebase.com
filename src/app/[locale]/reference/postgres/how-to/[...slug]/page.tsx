...
export function generateStaticParams() {
  const posts = getAllPosts(DIR_PATH);
...
export default function DocPage({ params }: { params: { slug: string[] } }) {
  ...
  const post = getPostBySlug(DIR_PATH, currentSlug);

  if (!post) return notFound();

  const { sidebar } = getSidebar(DIR_PATH);
  const flatSidebar = getFlatSidebar(sidebar);

  const breadcrumbs = getBreadcrumbs(DIR_PATH, currentPath, flatSidebar);
