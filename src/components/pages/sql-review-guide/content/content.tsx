...
const Content = ({
  ...
  <article className={className}>
    <h2 className="text-44 font-bold leading-tight text-gray-15 lg:text-36">
      Database Review Guide
    </h2>
    <div className="mt-[60px] flex flex-col gap-y-[60px] lg:mt-14">
      {categoryList.map(({ id, ruleList }) => {
        const lowerCaseId: string = id.toLocaleLowerCase();
        return (
          <div key={id}>
            <h3 className="text-30 font-semibold leading-tight text-gray-15 lg:text-24">
            ...
            <ul className="mt-8 lg:mt-6">
              {ruleList.map(({ type, engineList, componentList, level }) => {
                ...
                return (
                  <li className="border-t border-gray-90 pt-5 pb-8 last:pb-0 lg:pb-6" key={type}>
    ...
    <span className="mt-20 inline-flex w-full items-center border-t border-gray-90 pt-8 text-18 font-medium leading-none lg:mt-16">
      Made by{' '}
      <img
        className="mx-2"
