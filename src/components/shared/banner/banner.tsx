...
const Banner = ({ bannerText, bannerUrl }: { bannerText: string; bannerUrl: string }) => {
  ...
  return (
    ...
    >
      <Link
        className="group/link relative -z-10 mx-auto flex h-10 w-full items-center justify-center px-4.5 md:h-[40px]"
        href={bannerUrl}
      >
        <p className="line-clamp-1 text-center text-16 leading-tight transition-colors duration-200 md:text-14 2xs:text-left">
          {bannerText}
        </p>
