...
const getId = (children: ReactNode): string => {
  ...
  // Find the base ID without any numeric suffix
  let cleanBaseId = baseId;
  const match = baseId.match(/^(.+)-(\d+)$/);
  if (match && generatedIds.has(baseId)) {
    // This ID was generated by us, so use the original base
    cleanBaseId = match[1];
  }

  let counter = 1;
  let uniqueId = `${cleanBaseId}-${counter}`;
  while (usedIds.has(uniqueId)) {
    counter++;
    uniqueId = `${cleanBaseId}-${counter}`;
  }
