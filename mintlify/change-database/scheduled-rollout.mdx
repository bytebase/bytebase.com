...
- Clearly specify rollout timing requirements in:
...
1. **Issue Creation**: Developer creates issue with migration script and timing requirements
2. **Review & Approval**: Approver reviews the issue and timing constraints
3. **Schedule Decision**: Approver/Releaser determines optimal rollout time
4. **Execution**: Scheduled rollout runs automatically at the designated time

This workflow ensures proper separation of responsibilities while maintaining clear communication about rollout timing requirements.
