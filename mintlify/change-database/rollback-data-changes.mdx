...
Bytebase provides **Prior Backup** functionality that captures a snapshot of data before changes are applied. This safety mechanism stores the original state of affected rows, enabling you to revert data changes with a single click when necessary. You can perform **multi-task rollbacks** to efficiently revert multiple related changes across databases in one operation.
...
### Prerequisites for Prior Backup

Prior backup functionality requires **all** of the following conditions to be met:

- **Statement size**: SQL statement must be smaller than 2MB
- **Single operation type**: Cannot mix `UPDATE` and `DELETE` operations on the same table
- **DML only**: Cannot combine DDL (Data Definition Language) and DML (Data Manipulation Language) statements

### Prerequisites for 1-Click Rollback

1-click rollback capability requires **all** of the following conditions:

- **Primary key requirement**: For `UPDATE` operations, the target table must have a primary key
- **Primary key integrity**: Primary key columns cannot be modified during the update operation
