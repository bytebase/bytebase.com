...
| Database | Objects |
| -------- | ------- |
| **MySQL** | `Table`, `Partitioning` (KEY, HASH, RANGE, LIST), `View`, `Column`, `Primary Key`, `Foreign Key`, `Index` (Regular, Unique, Fulltext, Spatial), `Check Constraint` (8.0+), `Generated Column` (STORED/VIRTUAL), `Trigger`, `Function`, `Procedure`, `Comments` |
| **PostgreSQL** | `Schema`, `Table`, `Partitioning`, `View`, `Materialized View`, `Column`, `Primary Key`, `Foreign Key`, `Index` (including Expression & Partial), `Check Constraint`, `Generated Column` (STORED), `Sequence`, `Extension`, `Trigger`, `Function`, `Procedure`, `Custom Type`, `Enum Type`, `Array Type` |
| **TiDB** | `Table`, `Partitioning`, `View`, `Column`, `Primary Key`, `Foreign Key`, `Index`, `Check Constraint`, `Generated Column`, `AUTO_RANDOM`, `Clustered Index`, `Comments` |
| **Oracle** | `Schema`, `Table`, `Partitioning` (RANGE, LIST, HASH, COMPOSITE), `View`, `Materialized View`, `Column`, `Primary Key`, `Foreign Key`, `Index` (Regular, Unique, Bitmap, Function-based), `Check Constraint`, `Virtual Column`, `Identity Column` (12c+), `Trigger`, `Function`, `Procedure`, `Package`, `Sequence`, `Synonym`, `Type` |
| **Microsoft SQL Server** | `Schema`, `Table`, `View`, `Column`, `Primary Key`, `Foreign Key`, `Index` (Clustered, Non-clustered), `Check Constraint`, `Computed Column`, `Identity Column`, `Trigger`, `Function`, `Procedure`, `Default Constraint`, `User-Defined Type` |
