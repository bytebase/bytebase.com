...
Add user resources to your `main.tf`:
...
Set up workspace-level roles. Note that project roles assigned at workspace level apply to ALL projects:
...
Set up project-specific permissions for `Project Two`. These roles only apply to this specific project, however, the project roles defined in workspace level will show in both `Project One` and `Project Two`:
...
1. Go to **IAM & Admin > Users & Groups** to see all created users and groups.
...
1. Go to **Project One**, then click **Manage > Members** on the left sidebar to see project-level permissions.

   - Group `QA Team` is assigned to each project as `Project Viewer` at workspace level.
   - The project is created by terraform service account, so by default, it's the `Project Owner`.

   ![project-1-members](/content/docs/tutorials/manage-database-access-control-with-terraform/bb-project-1-members.png)

1. Go to **Project Two**, then click **Manage > Members** on the left sidebar to see project-level permissions.

   - Group `QA Team` is assigned to each project as `Project Viewer` at workspace level.
   - `allUsers` and Group `Developer Team` are assigned to this project as `Project Developer`.
   - `Database Administrator 1` and `Database Administrator 2` are assigned to this project as `Project Owner`.

   ![project-2-members](/content/docs/tutorials/manage-database-access-control-with-terraform/bb-project-2-members.png)

   - Click the pen icon on the `Developer Team`, you will see there is also a `SQL Editor User` role limited to `public.employee`and `public.department` tables to this group.
...
1. Click **IAM & Admin > Users & Groups**, find `Developer 1`, click on it and edit its password.

   ![dev1-pw](/content/docs/tutorials/manage-database-access-control-with-terraform/bb-dev1-pw.png)

1. Log in Bytebase as `Developer 1`, you can only see `Project Two`. Click **SQL Editor** on the top right. Connect to `Prod > Prod Sample Instance > hr_prod`.

   - Double click `employee` table on the left.
     ![sql-editor-employee](/content/docs/tutorials/manage-database-access-control-with-terraform/bb-sql-editor-employee.png)

   - Double click `salary` table on the left.
...
You've completed the Bytebase Terraform tutorial series! You now have a fully configured Bytebase instance with:
