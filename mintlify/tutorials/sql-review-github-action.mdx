...
When modifying data in the database, it's crucial to ensure that the changes are both safe and accurate. Bytebase offers a feature called [SQL Review](/sql-review/overview/), which allows you to evaluate your SQL changes before they are applied to the database. SQL Review can be invoked from the Bytebase GUI, API or CI.
...
Configure ngrok and create service account same as [Database GitOps with GitHub Actions](/tutorials/gitops-github-workflow/#step-1-start-bytebase-with-ngrok).
...
Now you have learned how to trigger SQL Review in GitHub Actions, you may also refer to the [SQL Review](/sql-review/overview) document for more details.
