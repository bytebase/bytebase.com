...
```hcl 8-3-global-data-masking.tf
...
1. Log in as Developer 1 (`dev1@example.com`), then go to **SQL Editor** to access `hr_prod`. Double-click `employee` table on the left. `birth_date` has `Date year mask` semantic type, and `last_name` has `Last Name Only Show First Letter`. Hovering the eye icon will show the masking reason.
...
|                       |                                                                                                                        |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Terraform resource    | [bytebase_database](https://registry.terraform.io/providers/bytebase/bytebase/latest/docs/resources/database)             |
...
1. After the expiration time, the masking exceptions will be removed.
