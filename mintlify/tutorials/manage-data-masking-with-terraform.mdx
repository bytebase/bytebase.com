...
1. Log in as Developer 1 (`dev1@example.com`), then go to **SQL Editor** to access `hr_prod`. Double-click `employee` table on the left. `birth_date` has `Mask Birth Date Year` semantic type, and `last_name` has `Last Name Only Show First Letter`. Hovering the eye icon will show the masking reason.
