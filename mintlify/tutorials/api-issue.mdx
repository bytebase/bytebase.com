...
1. Clone [Bytebase API Example
   repo](https://github.com/bytebase/example-api) and navigate to the `issue-creation` folder.
...
Congratulations! You've successfully created a schema change via Bytebase API. In similar ways, you could integrate Bytebase API into your existing DevOps platform to automate your schema change process, and benefit Bytebase features such as [SQL Review](/sql-review/review-policy/), [Custom Approval](/change-database/approval/) and [Schema Drift Detection](/change-database/drift-detection/).
