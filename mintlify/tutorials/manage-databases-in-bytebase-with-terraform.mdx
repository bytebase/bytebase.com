...
Learn how to automate database infrastructure management using [Terraform](https://www.hashicorp.com/products/terraform) and the [Terraform Bytebase Provider](https://registry.terraform.io/providers/bytebase/bytebase/latest/docs).
...
- **Bytebase Enterprise Plan** (optional) – Some features require the Enterprise Plan. You can follow this tutorial with the Community Plan by adjusting the configuration.
...
1. (Optional) After logging into Bytebase, click **Settings > Subscription** on the left sidebar, paste your Enterprise Plan license and click **Upload License**. _Some features require the Enterprise Plan. You can follow this tutorial with the Community Plan by adjusting the configuration._
...
1. Click **Instances** on the left sidebar to see two PostgreSQL instances: `Prod Sample Instance` and `Test Sample Instance`.
...
1. Within the project, click **Database > Databases** to see two databases: `hr_prod` (on `Prod Sample Instance`) and `hr_test` (on `Test Sample Instance`).

   ![add-db](/content/docs/tutorials/manage-databases-in-bytebase-with-terraform/bb-add-db.webp)

Manual processes work fine for a few instances, but Terraform enables you to manage hundreds of instances consistently across multiple environments.
