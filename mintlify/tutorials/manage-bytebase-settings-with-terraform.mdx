...
## Step 4 - Implement Dependencies and Apply Configuration

To ensure proper order of operations, set up dependencies between your settings:

```hcl
# Ensure workspace profile is configured first
resource "bytebase_setting" "workspace_profile" {
  name = "settings/WORKSPACE_PROFILE"
...
# Configure approval flow after workspace profile
resource "bytebase_setting" "approval_flow" {
  depends_on = [
...
# Configure risk policies after approval flow
resource "bytebase_risk" "prod_ddl_risk" {
  depends_on = [
    bytebase_setting.approval_flow
  ]

  name        = "Production DDL Risk"
  risk_level  = "HIGH"
...
1. **Risk Level Alignment**: Ensure your risk policies align with your approval flow requirements
2. **Environment-Based Policies**: Create stricter policies for production environments
3. **Gradual Implementation**: Start with basic policies and gradually add more specific rules
4. **Regular Review**: Periodically review and update your risk policies based on operational experience

## Summary

You've successfully configured Bytebase's core governance settings using Terraform. You've:

- **Configured workspace profile** settings for team collaboration
- **Set up approval flows** that automatically trigger based on risk levels
- **Defined risk management policies** that assess database operations
- **Implemented proper dependencies** to ensure correct configuration order

These settings provide a foundation for database governance that automatically enforces your organization's policies and procedures.

## Next Steps

With your Bytebase workspace fully configured, you can now:

- **Monitor Operations**: Use the Risk Center to track high-risk operations
- **Customize Policies**: Adjust risk policies based on your team's operational patterns
- **Extend Governance**: Add more specific rules for special cases or compliance requirements
- **Integrate with CI/CD**: Incorporate these governance controls into your deployment pipelines

You can also explore:

- [Data masking policies](https://www.bytebase.com/docs/security/data-masking) for sensitive data protection
- [Database branching](https://www.bytebase.com/docs/change-database/branch) for safer development workflows
- [Custom roles and permissions](https://www.bytebase.com/docs/administration/custom-roles) for fine-grained access control
