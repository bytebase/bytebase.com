...
- **Invalid filter syntax**: Ensure parentheses are balanced, e.g. `(&(objectClass=user)(uid=%s))`
- **Missing %s placeholder**: The `%s` must be present in the filter
- **Wrong attribute name**: Verify the username attribute (common ones: `uid`, `sAMAccountName`, `cn`, `mail`)
- **Wrong object class**: Check your LDAP schema to confirm the correct object class
