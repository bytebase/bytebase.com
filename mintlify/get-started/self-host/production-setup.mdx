...
store the metadata in an [external PostgreSQL database](/get-started/production-setup/#store-metadata-in-external-postgresql) and make sure the PostgreSQL instance is highly available.
...
Bytebase is now stateless and does not require persistent volumes. All metadata should be stored in an [external PostgreSQL database](/get-started/install/external-postgres).
