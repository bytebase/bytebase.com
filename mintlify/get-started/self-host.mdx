...
**Latest release version:** [**3.7.0**](https://github.com/bytebase/bytebase/releases/latest)
...
<Columns cols={2}>
  ...
  <Card title="Build from Source" href="#build-from-source" icon="code" />
</Columns>
...
- If accessing from China, pull image `registry.cn-shanghai.aliyuncs.com/bytebase/bytebase:3.7.0`.
...
docker run --init \
  ...
  --restart always \
  --publish 80:8080 \
  --volume ~/.bytebase/data:/var/opt/bytebase bytebase/bytebase:3.7.0 \
...
1. Go to the service you just created and click **Logs** tab to see the logs. If you see something like `Version 3.7.0(f5891735834ea82fd67bf8181597f86a7f5dab58) has started on port 8080 🚀`, it means Bytebase is running.
...
If you want to build from a specific release such as `3.7.0`, then switch to that release tag.
