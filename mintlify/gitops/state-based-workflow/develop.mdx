...
1. **Export** - Click `Export Schema` → `Multi-File (ZIP)` in Bytebase
2. **Extract** - Unzip the downloaded file to your Git repository
3. **Review** - The exported structure follows the [multi-file format](#file-organization)
...
- **Views** - Simple and materialized views
- **Functions & Procedures** - Stored procedures and user-defined functions
- **Sequences** - Independent sequences consolidated per schema
- **Indexes** - Standalone indexes (for materialized views)
- **Comments** - All object and column comments

All objects follow [SDL syntax requirements](#sdl-syntax-requirements) and are organized by schema and type.
