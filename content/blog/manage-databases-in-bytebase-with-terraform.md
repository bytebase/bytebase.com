...
feature_image: /blog/manage-databases-in-bytebase-with-terraform/terraform-feature.webp
tags: Tutorial
...
3. Register admin account `DBA`. This account will be granted `Workspace Owner` role. [https://www.bytebase.com/docs/concepts/roles-and-permissions](https://www.bytebase.com/docs/concepts/roles-and-permissions)
   ![register-admin](/blog/manage-databases-in-bytebase-with-terraform/register-admin.webp)
...
1. Follow the onboard guide or click **Add instance** on home page.
   ![add-instance](/blog/manage-databases-in-bytebase-with-terraform/add-instance.webp)

2. Create an instance for `Test` Environment with the following configuration. Fill **username**/**password** as `root`/`testpwd1`
   ![create-instance](/blog/manage-databases-in-bytebase-with-terraform/create-instance.webp)

3. Follow the onboard guide or click **New Project** on Projects page. Create a project `Test` with key `TEST` and click **Create**.
   ![create-project](/blog/manage-databases-in-bytebase-with-terraform/create-project.webp)

4. Follow the onboard guide or click **New DB** on the project `Test` page.
   ![add-new-db](/blog/manage-databases-in-bytebase-with-terraform/add-new-db.webp)
...
![create-db-demo](/blog/manage-databases-in-bytebase-with-terraform/create-db-demo.webp)

![issue-db-demo](/blog/manage-databases-in-bytebase-with-terraform/issue-db-demo.webp)
...
2. Go to [https://registry.terraform.io/providers/bytebase/bytebase/latest/docs](https://registry.terraform.io/providers/bytebase/bytebase/latest/docs). Click **Use Provider**, copy and paste the whole code block in the gray box into `main.tf`. Pay attention to the **version**.
   ![terraform-registration](/blog/manage-databases-in-bytebase-with-terraform/terraform-registration.webp)

3. Follow the document and go to **Example Usage**. Copy the following provider part and paste it in `main.tf`.
   ![terraform-code-version](/blog/manage-databases-in-bytebase-with-terraform/terraform-code-version.webp)
...
2. Turn on **Create as service account**, fill email with prefix `tf`, and click **+ Add**.
   ![bb-add-tf](/blog/manage-databases-in-bytebase-with-terraform/bb-add-tf.webp)

3. Scroll down, and you can see the newly added account there. Click **Copy Service Key**.
   ![bb-copy-tf-key](/blog/manage-databases-in-bytebase-with-terraform/bb-copy-tf-key.webp)

![bb-key-copied](/blog/manage-databases-in-bytebase-with-terraform/bb-key-copied.webp)
...
1. Paste the **Service Key**, **Service Account Email**, and **URL** into `main.tf`.The file now should look like this:
   ![vscode-tf-configure](/blog/manage-databases-in-bytebase-with-terraform/vscode-tf-configure.webp)
...
![environments-test](/blog/manage-databases-in-bytebase-with-terraform/environments-test.webp)
...
![instance-test](/blog/manage-databases-in-bytebase-with-terraform/instance-test.webp)
...
2. Run `terraform init`, `terraform plan` and `terraform apply` one by one in the terminal. You will see this in the terminal.
   ![terminal-ft-apply](/blog/manage-databases-in-bytebase-with-terraform/terminal-ft-apply.webp)

3. Go back to Bytebase, and click **Environments**. There is nothing changed with these two environments.
   ![environments-test-prod](/blog/manage-databases-in-bytebase-with-terraform/environments-test-prod.webp)
...
4. Click **Instances**. You’ll see there are two instances added.
   ![2-instances-tf](/blog/manage-databases-in-bytebase-with-terraform/2-instances-tf.webp)
...
1. Click **Projects** on the top navigation bar, and then click New Project. Create a new project called `TestTF`.
   ![create-proj-tf](/blog/manage-databases-in-bytebase-with-terraform/create-proj-tf.webp)

2. Click TestTF on the left side bar, and click **New DB**.
   ![testtf-new-db](/blog/manage-databases-in-bytebase-with-terraform/testtf-new-db.webp)
...
- **Instance**: MySQL Test TF
  ![demotf-create-db](/blog/manage-databases-in-bytebase-with-terraform/demotf-create-db.webp)

4. You’ll be redirected to the issue page, and click **Resolve issue**.
   ![issue-demotf-done](/blog/manage-databases-in-bytebase-with-terraform/issue-demotf-done.webp)

5. Click **Instances** on the top navigation bar, then select `MySQL Test TF`. You’ll see `demotf`.
   ![test-tf-db](/blog/manage-databases-in-bytebase-with-terraform/test-tf-db.webp)
