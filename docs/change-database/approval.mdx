...
1. **Condition Matching**: When a database change is created, Bytebase evaluates the approval flows in order under the matching source type. The first flow whose condition matches is used
2. **Fallback**: If no source-specific flow matches, fallback flows are evaluated
...
<Info>
For **Change Database**, a `risk_level` variable (`"HIGH"`, `"MODERATE"`, `"LOW"`) is also available. It is automatically calculated by Bytebase based on the statement type.
</Info>
